            ...
            window.Echo.private(`vendor-channel.${window.appConfig.vendorUid}`).listen('.VendorChannelBroadcast', function (data) {
                ...
                broadcastActionDebounce = setTimeout(function() {
                    ...
                    if($('.lw-whatsapp-chat-window').length) {
                        __DataRequest.get(__Utils.apiURL("{{ route('vendor.contacts.data.read', ['contactUid']) }}{{ ((isset($assigned) and $assigned) ? '?assigned=to-me' : '') }}", {'contactUid': $('#lwWhatsAppChatWindow').data('contact-uid')}),{}, function() {});
                    }
                    // chat updates
                    if(data.contactUid && $('[data-contact-uid=' + data.contactUid + ']').length) {
                            __DataRequest.get(__Utils.apiURL("{{ route('vendor.chat_message.data.read', ['contactUid']) }}{{ ((isset($assigned) and $assigned) ? '?assigned=to-me' : '') }}", {'contactUid': data.contactUid}),{}, function() {
