...
namespace App\Yantrana\Components\AutoCampaign\Repositories;

use App\Yantrana\Base\BaseRepository;
use App\Yantrana\Components\Campaign\Interfaces\CampaignRepositoryInterface;
use App\Yantrana\Components\AutoCampaign\Models\AutoCampaignModel;
...
class AutoCampaignRepository extends BaseRepository implements CampaignRepositoryInterface
{
    /**
     ...
     * @var object
     */
    protected $primaryModel = AutoCampaignModel::class;
     ...
     * @return mixed
     *---------------------------------------------------------------- */
    public function fetchAutoCampaignDataTableSource($status)
    {
        if($status=="archived"){
            $status=5;
        }else{
        ...
        return $this->primaryModel::where([
            ...
            $query->whereNotNull('campaigns__id');
        })
            ->withCount([
            'messageLog',
