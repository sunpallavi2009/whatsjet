...
<x-lw.datatable lw-card-classes="border-0" data-page-length="100" id="lwCampaignQueueLog" :url="route('vendor.campaign.executed.log.list.view', ['campaignUid' => $campaignUid])">
    ...
    <th data-orderable="true" data-template="#campaignStatus" data-name="status">{{ __tr('Status') }}</th>
...
<script type="text/template" id="campaignStatusMessage">
    <% if (__tData.status == 'failed') { %>
        <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{  __tr('Failed') }}</span>
...
<script type="text/template" id="campaignStatus">
    <div><%- __tData.messaged_at %></div>
    <% if (__tData.status == 'failed') { %>
