...
*/
@endphp
@extends('layouts.app', ['title' => __tr('AutoCampaigns')])
@section('content')
@include('users.partials.header', [
'title' => __tr('AutoCampaigns'),
'description' => '',
'class' => 'col-lg-7'
])
...
<div class="container-fluid mt-lg--6">
    <div class="row">
        <div class="col-xl-12 mb-3">
            <button class="lw-btn btn btn-secondary float-right mr-2" id="fetchDataButton">{{ __tr('Payment Reminder') }}</button>
        </div>
        <!-- button -->
        <div class="col-xl-12 mb-3">
        ...
        <div class="col-xl-12">
            <x-lw.datatable data-page-length="100" id="lwCampaignList" :url="route('vendor.autocampaign.read.list', ['status' => $status])">
...
<script>
    ...
    var baseUrl = '{{ route('vendor.autocampaign.process.data') }}';

    $('#fetchDataButton').on('click', function() {
        $.ajax({
            url: '{{ route('vendor.autocampaign.getNextCampaignId') }}',
            method: 'GET',
            success: function(response) {
                var nextCampaignId = response.nextCampaignId;
                var url = baseUrl + '?campaigns__id=' + nextCampaignId;
                ...
                fetch(url)
                    ...
                    .then(data => {
                        if (data.message) {
                            ...
                            setTimeout(function() {
                                location.reload();
                            }, 2000); // Delay reload for 2 seconds
                        } else if (data.error) {
                            toastr.error(data.error);
                        }
                    })
                    .catch(error => {
                        toastr.error('Error fetching and sending data: ' + error);
                    });
            },
