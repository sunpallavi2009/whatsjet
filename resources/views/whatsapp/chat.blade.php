    ...
    <div class="">
        <div class="card lw-whatsapp-chat-block-container">
            ...
            <div id="lwWhatsAppChatWindow"
                class="card-body lw-whatsapp-chat-window p-sm-4" :data-contact-uid="contact?._uid">
                <div class="row" x-cloak x-data="{isContactListOpened:false,isContactCrmBlockOpened:false}">
                    <div class="col-sm-12 col-md-3 col-lg-3 col-xl-3 mb-4 lw-contact-list-block" x-show="isContactListOpened">
                        ...
                        <div class="form-group m-0"><label for="lwShowUnreadOnlyContacts"><input data-lw-plugin="lwSwitchery" data-color="orange" data-size="small" x-model="showUnreadContactsOnly" class="custom-checkbox" id="lwShowUnreadOnlyContacts" type="checkbox" name="unread_only_contacts" id=""> <span x-show="!showUnreadContactsOnly">{{  __tr('Show all') }}</span><span x-show="showUnreadContactsOnly">{{  __tr('Show unread only') }}</span></label></div>
                        <nav>
                          ...
                          <div class="tab-content" id="nav-tabContent" x-cloak>
                            ...
                            <template x-for="contactItem in filteredContacts">
                                ...
                                @endif
                                <a x-show="(contact && contact._uid == contactItem._uid) || (showUnreadContactsOnly && contactItem.unread_messages_count) || !showUnreadContactsOnly" @click.prevent="isContactListOpened = false" :data-messaged-at="contactItem.last_message?.messaged_at"
...
<script>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('initialMessageData', () => ({
            ...
            assignedLabelIds: @json($assignedLabelIds),
            ...
            filteredContacts: function () {
                ...
                return _.filter(this.contacts, (item) => {
                    return (item.full_name + item.wa_id + item.label_string)
                    .toLowerCase()
            ...
            labelsElement : function() {
                // reset the selectize
                if($('#lwAssignLabelsField')[0].selectize) {
                    $('#lwAssignLabelsField')[0].selectize.destroy();
                }
...
(function($) {
    ...
    window.onUpdateContactDetails = function(responseData, callbackParams) {
        __DataRequest.get(__Utils.apiURL('{{ route('vendor.chat_message.contact.view', ['contactUid', 'assigned' => (($assigned ?? null) ? 'to-me' : '')]) }}',{'contactUid': $('#lwWhatsAppChatWindow').data('contact-uid')}),{}, function() {});
        if(callbackParams) {
